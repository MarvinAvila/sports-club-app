...
const ProfileDropdown = ({ user, role = 'Usuario' }) => {
  const { logout, user: authUser } = useAuth(); // Asegúrate de que useAuth devuelva los datos del usuario
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="flex items-center">
          ...
          <FiChevronDown className={`ml-1 text-gray-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
        </div>
      </button>

      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-gray-700 rounded-lg shadow-xl z-50 border border-gray-600 divide-y divide-gray-600 overflow-hidden animate-fade-in">
          {/* Encabezado con información del usuario */}
          <div className="p-4 bg-gradient-to-r from-purple-600/20 to-blue-600/20">
            <div className="flex items-center space-x-3">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-1">
                  <span className="text-xs px-2 py-1 bg-gray-600/50 text-gray-200 rounded-full inline-block">
                    {role}
                  </span>
                </div>
              </div>
          ...
          {/* Menú principal con mejor espaciado */}
            ...
            >
              <div className="flex items-center">
                {theme === 'dark' ? (
                  ...
                  <FiSun className="w-4 h-4 mr-3 text-yellow-400" />
                )}
                <span>Tema</span>
              </div>
          ...
          {/* Cerrar sesión con estilo destacado */}
          <div className="py-2">
            <button 
              onClick={handleLogout}
              className="w-full flex items-center px-4 py-2 text-sm text-red-400 hover:bg-gray-600 transition-colors"
            >
...
const MenuItem = ({ icon, text }) => (
  <a href="#" className="flex items-center px-4 py-2 text-sm text-gray-300 hover:bg-gray-600 transition-colors">
